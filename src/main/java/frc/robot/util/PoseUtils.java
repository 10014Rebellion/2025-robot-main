...
public class PoseUtils {
  ...
  /**
   * Shifts a given pose by a specified distance forward and right relative to the pose's current
   * orientation.
   *
   * @param originalPose The original pose to shift.
   * @param forwardMeters The distance to shift forward, relative to the pose's heading.
   * @param rightMeters The distance to shift right, relative to the pose's heading.
  ...
  public static Pose2d shiftPose(Pose2d originalPose, double forwardMeters, double rightMeters) {
    ...
    Translation2d translation =
        new Translation2d(forwardMeters, rightMeters).rotateBy(originalPose.getRotation());
  ...
  /**
   * Rotates a given pose by a specified rotation and then shifts it by a specified distance forward
   * and right relative to the rotated pose's new orientation.
   *
   * @param originalPose The original pose to rotate and shift.
   * @param rotationDelta The amount of rotation to apply to the pose before shifting.
   * @param forwardMeters The distance to shift forward, relative to the rotated pose's heading.
   * @param rightMeters The distance to shift right, relative to the rotated pose's heading.
  ...
  public static Pose2d shiftPose(
    ...
    Pose2d rotatedPose =
        new Pose2d(originalPose.getTranslation(), originalPose.getRotation().plus(rotationDelta));

    // Create a translation relative to the newly rotated orientation
    Translation2d translation =
        new Translation2d(forwardMeters, rightMeters).rotateBy(rotatedPose.getRotation());
