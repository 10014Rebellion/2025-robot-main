...
public class PoseCamera {
  ...
  public Pose3d getCameraPoseEstimate() {
    if (m_photonCamera.getAllUnreadResults().size() > 0) {
        return m_photonPoseEstimator.update(m_photonCamera.getAllUnreadResults().get(0)).get().estimatedPose;
    }
  ...
  public void updateTelemetry() {
    SmartDashboard.putBoolean("Vision/Pose/" + m_photonCamera.getName() + "/IsConnected", m_photonCamera.isConnected());
    SmartDashboard.putBoolean("Visions/Pose/" + m_photonCamera.getName() + "/HasTarget", m_photonCamera.getLatestResult().hasTargets());
  }
}
