...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  public void setVoltage(double pVolts, boolean pOverride) {
    if (canMove(pVolts) || pOverride) mElevatorMotor.setVoltage(MathUtil.clamp(pVolts, -12, 12));
    else stopAllMotors();
  }

  public void setPercentOutput(double pPercent, boolean pOverride) {
    if (canMove(pPercent) || pOverride) mElevatorMotor.set(MathUtil.clamp(pPercent, -1, 1));
    else stopAllMotors();
  }
  ...
  public void periodic() {
    ...
    if (!canMove(getPercentOutput())) stopAllMotors();
  }
}
