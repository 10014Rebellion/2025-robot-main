...
public class Vision extends SubsystemBase {
  ...
  private void UpdatePose() {
    ...
    for (PoseCamera camera : mCameraList) {
      ...
      if (results.size() > 0) {
        camera.getEstimatedRobotPose(results.get(0)).ifPresent(estimatedRobotPose -> {
          ...
          if (isPoseOnField(pose)) {
            if (estimatedRobotPose.strategy == PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR) {
              m_poseEstimator.addVisionMeasurement(pose, estimatedRobotPose.timestampSeconds, VisionConstants.kVisionMultiTagStandardDeviations);
            } else {
              for (PhotonTrackedTarget target : estimatedRobotPose.targetsUsed) {
                if (MiscUtils.isValueInRange(target.getPoseAmbiguity(), 0.0, VisionConstants.kVisionMaxPoseAmbiguity)) {
                  m_poseEstimator.addVisionMeasurement(pose, estimatedRobotPose.timestampSeconds, VisionConstants.kVisionSingleTagStandardDeviations);
                  break;
                }
              }
            }
  ...
  private boolean isPoseOnField(Pose2d pose) {
    double x = pose.getX();
    double y = pose.getY();
    return (x >= 0.0 && x <= VisionConstants.kAprilTagFieldLayout.getFieldLength()) && (y >= 0.0 && y <= VisionConstants.kAprilTagFieldLayout.getFieldWidth());
  }
  ...
  public void UpdateTelemetry() {
    ...
    for (PoseCamera camera : mCameraList) {
      Logger.recordOutput("Vision/Pose/" + camera.getCameraName(), camera.getCameraPoseEstimate());
    }
